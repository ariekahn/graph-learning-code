...
# The FEAT localizer is identifying the
# Lateral Occipital Cortex by identifying
# voxels that show a contrast between
# shapes and phase-scrambled images
...
TEMPLATE="$PROJECT_DIR/extra/feat_localizer_template.fsf"
...
for report in $PROJECT_DIR/derived/fmriprep/*.html; do
    ...
    FSL_TEMP_FILE="$FSL_TEMP_DIR/${subject}_feat_localizer.fsf"
    CONFOUNDS_DIR=$PROJECT_DIR/derived/fsl/$subject/confounds
    EVENTS_DIR=$PROJECT_DIR/derived/fsl/$subject/events
    OUTPUT_DIR=$PROJECT_DIR/derived/fsl/$subject/localizer
    sed "s/SUBJECT/$subject/g; s:FSLDIR:$FSLDIR:g; s:FMRIPREP_DIR:$FMRIPREP_DIR:g; s:CONFOUNDS_DIR:$CONFOUNDS_DIR:g; s:EVENTS_DIR:$EVENTS_DIR:g; s:OUTPUT_DIR:$OUTPUT_DIR:g" $TEMPLATE > $FSL_TEMP_FILE
