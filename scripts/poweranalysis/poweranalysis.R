...
# Table: Cross-Cluster Surprisal Effect Modeling
# Old data, experiment 1

surprisal.data <- task.orig %>%
  filter(valid == TRUE) %>%
  filter(format == "walktest") %>%
  filter(stage == 1) %>%
  filter(graph == "Modular")
...
compute.sample <- function(x, n.samples) {
  subjects.surprisal.data <- unique(surprisal.data$subject)
  subjects.sample <- sample(subjects.surprisal.data, n.samples, replace=TRUE)
  sample.data.list <- list()
  for (i in 1:n.samples) {
    sample.data.list[[i]] <- surprisal.data %>%
      filter(subject == subjects.sample[i]) %>%
      mutate(subject.num = i)
  }
  sample.data <- bind_rows(sample.data.list)
  sample.surprisal.model <- lmer(rt ~ target + z.log.trial*transition_in_1 + (1 + z.log.trial*transition_in_1 | subject.num), 
                                 sample.data)
  return(get_confint(sample.surprisal.model)[17,1:8])
}

n.sample.runs <- 10000
cl <- makeForkCluster(nnodes=n.threads)
clusterExport(cl, list("get_confint", "get_confint_summary", "compute.sample", "surprisal.data"))
