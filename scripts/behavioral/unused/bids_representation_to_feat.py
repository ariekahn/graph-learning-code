...
parser = argparse.ArgumentParser('Create FSL-style representation node event files')
...
for run in range(1,9):
	events = pd.read_csv(f'{scan_dir}/sub-{subject_id}_ses-2_task-graphrepresentation_run-{run}_events.tsv', sep='\t')
	events['value'] = 1
	for node in range(15):
		...
		filename = f'{output_dir}/sub-{subject_id}_ses-2_task-graphrepresentation_run-{run}_node-{node}.txt'
