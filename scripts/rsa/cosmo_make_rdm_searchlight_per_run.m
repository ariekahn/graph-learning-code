function cosmo_make_rdm_searchlight_per_run(subject, runidx, project_path, output_path, nproc)
...
runidx = str2num(runidx);

diary(fullfile(output_path, sprintf('sub-%s_run-%d.txt', subject, runidx)));
...
%% Combined mask
bold_mask_path = fullfile(fmriprep_func_path, sprintf('sub-%s_ses-2_task-graphrepresentation_run-%d_space-MNI152NLin2009cAsym_desc-brain_mask.nii.gz', subject, runidx));
load_nii(bold_mask_path); % Only here to get mcc to include fn - need to fix
%% Load Data
disp('Loading LS-S data');
disp('zfiles');
ds_lss_zfiles_all = cell(59, 1);
counter = 0;
...
ds_lss_zfiles_zscore = cosmo_fmri_dataset(ds_lss_zfiles);
ds_lss_zfiles_zscore.samples = ds_lss_zfiles_zscore.samples - mean(ds_lss_zfiles_zscore.samples);
ds_lss_zfiles_zscore.samples = ds_lss_zfiles_zscore.samples ./ std(ds_lss_zfiles_zscore.samples);
...
ds_cf_lss_zfiles_zscore = cosmo_searchlight(ds_lss_zfiles_zscore_avg, nbrhood_lss_zfiles_zscore, measure, measure_args, 'nproc', nproc);

%% Save results
output_fn=fullfile(output_path, 'results', sprintf('sub-%s_run-%d_rdm_nvox-%.0f_searchlight-lss-zfiles.nii.gz', subject, runidx, nvoxels_per_searchlight));
cosmo_map2fmri(ds_cf_lss_zfiles, output_fn);

output_fn=fullfile(output_path, 'results', sprintf('sub-%s_run-%d_rdm_nvox-%.0f_searchlight-lss-zfiles-zscored.nii.gz', subject, runidx, nvoxels_per_searchlight));
cosmo_map2fmri(ds_cf_lss_zfiles_zscore, output_fn);
