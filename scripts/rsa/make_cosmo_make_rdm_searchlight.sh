...
OUTPUT_DIR=$PROJECT_DIR/derived/cosmo_make_rdm_searchlight-${DATE}
mkdir -p $OUTPUT_DIR
...
MEMORY_GB=30.0
# Output script name
SCRIPT_BASE_NAME=cosmo_make_rdm_searchlight.sh
...
#!/bin/bash
set -euo pipefail
if [[ cosmo_make_rdm_searchlight.m -nt cosmo_make_rdm_searchlight ]]; then
  echo .m file is newer than mcc output
  echo run \\\`mcc -m cosmo_make_rdm_searchlight.m\\\`
  exit 1
fi
EOF
...
LD_PRELOAD=/cbica/software/external/matlab/R2018A/sys/os/glnxa64/libstdc++.so.6.0.22 ./cosmo_make_rdm_searchlight $subject $PROJECT_DIR $OUTPUT_DIR/sub-$subject $N_CORES
