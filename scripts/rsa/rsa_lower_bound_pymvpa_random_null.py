...
parser.add_argument('seed', type=int, help='Random permutation seed')
...
logging.basicConfig(filename=f'{output_dir}/rsa_lower-bound_ordering-random-{seed}.log', filemode='w', level=logging.INFO)
...
node_to_random = dict()  # [random0, random1, etc]
random_to_node = dict()
node_to_random_square = dict()  # [random0, random1, etc]
random_to_node_square = dict()
for subject in subjects:
    filename = f'{data_dir}/sub-{subject}/ses-2/func/sub-{subject}_ses-2_task-graphrepresentation_run-1_events.tsv'
    data = pd.read_csv(filename, sep='\t')
    node_to_random[subject] = rg.permutation(15)
    random_to_node[subject] = np.argsort(node_to_random[subject])
    node_to_random_square[subject] = squareform(squareform(range(105))[node_to_random[subject], :][:, node_to_random[subject]])
    random_to_node_square[subject] = squareform(squareform(range(105))[random_to_node[subject], :][:, random_to_node[subject]])
    results[subject] = new_img_like(results[subject],
                                    results[subject].get_fdata()[:, :, :, node_to_random_square[subject]],
...
if comparison_metric == 'spearman':
    ...
    results_modular_img.to_filename(f'{output_dir}/rdm_lower-bound_pairwise-metric-{pairwise_metric}_radius-{radius}_comparison-metric-{comparison_metric}_ordering-random_seed-{seed}_modular.nii.gz')
    del results_modular
    ...
    results_lattice_img.to_filename(f'{output_dir}/rdm_lower-bound_pairwise-metric-{pairwise_metric}_radius-{radius}_comparison-metric-{comparison_metric}_ordering-random_seed-{seed}_lattice.nii.gz')
    del results_lattice
    ...
    results_all_img.to_filename(f'{output_dir}/rdm_lower-bound_pairwise-metric-{pairwise_metric}_radius-{radius}_comparison-metric-{comparison_metric}_ordering-random_seed-{seed}_all.nii.gz')
    del results_all
    del results_all_img
    gc.collect()
