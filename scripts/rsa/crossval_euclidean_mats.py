...
loc_rois = (
    ('loc-localized-both', mask_loc, argsort_zstat_masked, [200, 400, 600, 800]),
    ('loc-localized-lh', mask_loc_lh, argsort_zstat_masked_lh, [50, 100, 150, 200]),
    ('loc-localized-rh', mask_loc_rh, argsort_zstat_masked_rh, [50, 100, 150, 200]),
)


for (rescale, subtract_mean) in zstat_params:
    for (roi_base_name, mask, argsort_zstat, voxel_list) in loc_rois:
        f, ax = plt.subplots()
        plotting.plot_roi(mask_loc,
                          bg_img=template,
                          black_bg=False,
                          axes=ax,
                          title=f'{roi_base_name} / {subject_id} / {n_voxels_overlap} voxels)')
        f.savefig(f'{output_dir}/images/sub-{subject_id}_{roi_base_name}_mask.pdf')
        zstats_lss = load_zstats_lss(mask, sort_by=argsort_zstat, rescale=rescale, subtract_mean=subtract_mean)
        logging.info(f'zstats {roi_base_name} LS-S:')
        logging.info(f'{zstats_lss.shape[0]} events')
        logging.info(f'{zstats_lss.shape[1]} voxels')

        for nvoxels in voxel_list:
            roi_name = f'{roi_base_name}-{nvoxels}'

            block_avg_patterns = np.zeros((8, 15, nvoxels))
            for block, node in np.ndindex(8, 15):
                block_avg_patterns[block, node, :] = zstats_lss[(nodes_lss == node) & (blocks_lss == block), :nvoxels].mean(0)
            np.save(f'{output_dir}/sub-{subject_id}_rescale-{rescale}_subtract-mean-{subtract_mean}_cross-val-euclidean-squared_{roi_name}_zstats_block-avg-pattern.npy',
                    block_avg_patterns, allow_pickle=False)
            block_avg_results = np.zeros((15, 15))
            for node_i, node_j, block_i, block_j in np.ndindex(15, 15, 8, 8):
                if block_i != block_j:
                    # (bj - bk)_A * (bj -bk)_B^T
                    b1n1 = block_avg_patterns[block_i, node_i, :]
                    b1n2 = block_avg_patterns[block_i, node_j, :]
                    b2n1 = block_avg_patterns[block_j, node_i, :]
                    b2n2 = block_avg_patterns[block_j, node_j, :]
                    dist = np.inner((b1n1 - b1n2), (b2n1 - b2n2))
                    block_avg_results[node_i, node_j] += dist

            block_avg_results /= 8 * 7
            np.save(f'{output_dir}/sub-{subject_id}_rescale-{rescale}_subtract-mean-{subtract_mean}_cross-val-euclidean-squared_{roi_name}_zstats_block-avg.npy', block_avg_results, allow_pickle=False)

        gc.collect()

# Custom threshold images (for exploring ROIs)
def load_threshold_ctx(subject_id, region_id=0):
    aparc = load_aparc(subject_id)
    ctx = math_img('(img >= 1000) & (img < 3000)', img=aparc)
    thresh_map = load_img(f'{project_dir}/derived/template_threshold/threshold_region-{region_id}.nii.gz')
    img = math_img('img1 * img2', img1=ctx, img2=thresh_map)
    return img
...
for (rescale, subtract_mean) in zstat_params:
    rois = (
        ...
        ('threshold-ctx-0', partial(load_threshold_ctx, region_id=0)),
        ('threshold-ctx-1', partial(load_threshold_ctx, region_id=1)),
        ('threshold-ctx-2', partial(load_threshold_ctx, region_id=2)),
        ('threshold-ctx-3', partial(load_threshold_ctx, region_id=3)),
        ('threshold-ctx-4', partial(load_threshold_ctx, region_id=4)),
        ('threshold-ctx-5', partial(load_threshold_ctx, region_id=5)),
        ('threshold-ctx-6', partial(load_threshold_ctx, region_id=6)),
        ('threshold-ctx-7', partial(load_threshold_ctx, region_id=7)),
        ('threshold-ctx-8', partial(load_threshold_ctx, region_id=8)),
        ('threshold-ctx-9', partial(load_threshold_ctx, region_id=9)),
        ('threshold-ctx-10', partial(load_threshold_ctx, region_id=10)),
        ('threshold-ctx-11', partial(load_threshold_ctx, region_id=11)),
    )
