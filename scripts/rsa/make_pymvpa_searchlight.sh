...
OUTPUT_DIR=$PROJECT_DIR/derived/pymvpa_searchlight-${DATE}
mkdir -p $OUTPUT_DIR
...
MEMORY_GB=10.0
# Output script name
SCRIPT_BASE_NAME=pymvpa_searchlight.sh
...
source activate pymvpa
set -euo pipefail
python $PROJECT_DIR/scripts/rsa/pymvpa_searchlight.py $subject $PROJECT_DIR $OUTPUT_DIR/sub-$subject 3
conda deactivate
